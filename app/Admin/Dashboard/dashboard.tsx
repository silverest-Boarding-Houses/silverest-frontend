...
export default function AdminActions() {
  ...
  useEffect(() => {
    const fetchTotalAmount = async () => {
      try {
        const response = await fetch('https://silverestbackend-42mz.onrender.com/bookings/total-bookingfee-today');
  ...
  return (
    <div className="bg-gray-400 py-10 min-h-screen flex flex-col items-center px-4">
      {/* Total Amount Card */}
      <div className="fixed top-5 right-5 bg-white shadow-lg rounded-lg p-4 w-40 sm:w-56 md:w-64 text-center">
        <h3 className="text-sm sm:text-lg font-semibold mb-2">Total Amount Today</h3>
        <p className="text-lg sm:text-2xl font-bold text-green-600">ZK{totalAmount}</p>
      </div>

      {/* Actions Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full max-w-5xl mt-20 px-2 sm:px-4">
        {[{
          icon: HomeIcon,
          title: "Post a House",
        ...
        },
        {
          ...
          <div key={index} className="bg-white shadow-lg rounded-lg p-4 text-center flex flex-col items-center w-full">
            ...
            <a href={item.link} className="w-full">
              <button className={`${item.buttonColor} text-white px-4 py-2 w-full sm:w-auto rounded-md hover:bg-orange-600 transition`}>
                {item.buttonText}
              </button>
            </a>
          </div>
